...
data_low = pd.read_csv('第二问数据集/低级机器设备.csv')
...
def cv_model(clf, train_x, train_y, test_x, clf_name):
    ...
    for i, (train_index, valid_index) in enumerate(kf.split(train_x, train_y)):
        ...
        if clf_name == "lgb":
            ...
            joblib.dump(model, '模型保存/save_model(问题二lgb低级).pkl')
...
plt.title('低级机器二分类lgb测试集混淆矩阵可视化', fontsize=22)
...
model = joblib.load('模型保存/save_model(问题二lgb低级).pkl')
