...
data_low = pd.read_csv('第三问数据集/高等级机器设备.csv')
...
def cv_model(clf, train_x, train_y, test_x, clf_name):
    ...
    for i, (train_index, valid_index) in enumerate(kf.split(train_x, train_y)):
        ...
        if clf_name == "lgb":
            ...
            joblib.dump(model, '模型保存/save_model(问题三lgb高级).pkl')
...
# 计算混淆矩阵
cnf = confusion_matrix(true, pre, labels=[x for x in range(0, 4)])
cnf

# In[17]:


conf_matrix = pd.DataFrame(cnf, index=['Normal', 'HDF', 'PWF', 'TWF'], columns=['Normal', 'HDF', 'PWF', 'TWF'])
...
plt.title('高级机器多分类lgb测试集混淆矩阵可视化', fontsize=22)
...
plt.title('高级机器测试集评估指标可视化', fontsize=20)
...
model = joblib.load('模型保存/save_model(问题三lgb高级).pkl')
...
cnf = confusion_matrix(train_trueLabel, train_outcome, labels=[x for x in range(0, 4)])
cnf
