...
data = pd.read_csv(r"第三问数据集/高等级机器设备.csv")
...
# 保存模型
joblib.dump(rfc, '模型保存/save_model(问题三rf高级).pkl')
...
plt.title('高级机器多分类rf测试集混淆矩阵可视化', fontsize=12)
...
dot_data = export_graphviz(dtc
                           , feature_names=['转速（rpm）', '扭矩（Nm）', '功率（KW）', '厂房室温（℃）', '机器温度（℃）']
...
names = ["扭矩（Nm）", "功率（Kw）", "转速（rpm）", '厂房室温（K）', '机器温度（K）']
