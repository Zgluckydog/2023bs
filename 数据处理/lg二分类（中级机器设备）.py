...
def cv_model(clf, train_x, train_y, test_x, clf_name):
    ...
    for i, (train_index, valid_index) in enumerate(kf.split(train_x, train_y)):
        ...
        if clf_name == "lgb":
            ...
            joblib.dump(model, '模型保存/save_model(问题二lgb中级).pkl')
...
plt.title('中级机器设备二分类lgb测试集混淆矩阵可视化', fontsize=22)
...
plt.title('中级机器设备测试集评估指标可视化', fontsize=20)
...
model = joblib.load('模型保存/save_model(问题二lgb中级).pkl')
