# -*- coding = utf-8 -*-
# @Time : 2023/3/14 15:20
# @Author : 张贵
# @File : lg多分类（低级机器设备）.py
# @Software : PyCharm
...
data_low = pd.read_csv('第三问数据集/低等级机器设备.csv')
...
def cv_model(clf, train_x, train_y, test_x, clf_name):
    ...
    test = np.zeros((test_x.shape[0], 6))
    ...
    for i, (train_index, valid_index) in enumerate(kf.split(train_x, train_y)):
        ...
        if clf_name == "lgb":
            ...
            joblib.dump(model, '模型保存/save_model(问题三lgb低级).pkl')
...
# 计算混淆矩阵
cnf = confusion_matrix(true, pre, labels=[x for x in range(0, 6)])
cnf
...
plt.title('低级机器多分类lgb测试集混淆矩阵可视化', fontsize=22)
...
plt.title('低级机器测试集评估指标可视化', fontsize=20)
...
model = joblib.load('模型保存/save_model(问题三lgb低级).pkl')
...
cnf = confusion_matrix(train_trueLabel, train_outcome, labels=[x for x in range(0, 6)])
cnf
...
plt.xlabel('预测值', fontsize=18)
plt.title('低级机器设备二分类lgb训练集混淆矩阵可视化', fontsize=22)
plt.xticks(fontsize=18)
plt.yticks(fontsize=18)
plt.savefig('confusion.pdf', bbox_inches='tight')
plt.show()
...
aa.drop(['机器编号', '统一规范代码', '厂房室温（K）', '机器温度（K）'], axis=1, inplace=True)
...
plt.pie(nums, counterclock=False, startangle=20)
plt.title('低级机器设备等级数量分布图(饼图)')
plt.legend(labels=['Normal', 'HDF', 'TWF', 'OSF', 'PWF', 'RNF'])
plt.show()
