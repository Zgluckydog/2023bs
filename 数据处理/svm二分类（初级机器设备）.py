...
joblib.dump(classifier, '模型保存/save_model(问题二SVM低级).pkl')
...
plt.title('低级机器二分类SVM测试集混淆矩阵可视化', fontsize=22)
...
model = joblib.load('模型保存/save_model(问题二SVM低级).pkl')
model.predict(xtest)
