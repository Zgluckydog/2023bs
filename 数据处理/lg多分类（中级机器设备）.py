...
def cv_model(clf, train_x, train_y, test_x, clf_name):
    ...
    for i, (train_index, valid_index) in enumerate(kf.split(train_x, train_y)):
        ...
        if clf_name == "lgb":
            ...
            joblib.dump(model, '模型保存/save_model(问题三lgb中级).pkl')
...
plt.title('中级机器多分类lgb测试集混淆矩阵可视化', fontsize=22)
...
plt.title('中级机器测试集评估指标可视化', fontsize=20)
...
model = joblib.load('模型保存/save_model(问题三lgb中级).pkl')
...
plt.xlabel('预测值', fontsize=18)
plt.title('中级机器设备二分类lgb训练集混淆矩阵可视化', fontsize=22)
plt.xticks(fontsize=18)
plt.yticks(fontsize=18)
plt.savefig('confusion.pdf', bbox_inches='tight')
plt.show()
...
aa.drop(['机器编号', '统一规范代码'], axis=1, inplace=True)
...
plt.pie(nums, counterclock=False, startangle=20, labeldistance=1.1)
plt.title('中级机器设备等级数量分布图(饼图)')
plt.legend(labels=['Normal', 'PWF', 'TWF', 'HDF'])
plt.show()
